...
private:
    void printTreeHelper(TreeNode* root, int m, int lo, int hi, vector<vector<string>> &res) {
        if (!root )
            return;
        int mid = lo + (hi - lo) / 2;
        res[m][mid] = to_string(root->val);
        printTreeHelper(root->left, m + 1, lo, mid - 1, res);
        printTreeHelper(root->right, m + 1, mid + 1, hi, res);
    }
