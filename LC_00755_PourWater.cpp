...
public:
    vector<int> pourWater(vector<int>& heights, int V, int K) {
        while (V-- > 0)
            pourWaterHelper(heights, K);
        return heights;
    }
private:
    void pourWaterHelper(vector<int>& heights, int k) {
        for (int i = k - 1; i >= 0 && heights[i] <= heights[k]; i--) {
            if(heights[i] == heights[k])
                continue;
            pourWaterHelper(heights, i);
            return;
        }
        for (int i = k + 1; i < heights.size() && heights[i] <= heights[k]; i++) {
            if(heights[i] == heights[k])
                continue;
            pourWaterHelper(heights, i);
            return;
